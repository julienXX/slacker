...
update(Token, TimeStamp, Channel, Text) ->
    request:slack("chat.delete", [{"token", Token},{"ts", TimeStamp},{"channel", Channel},{"text", Text}]).
...
delete(Token, TimeStamp, Channel) ->
    request:slack("chat.delete", [{"token", Token},{"ts", TimeStamp},{"channel", Channel}]).

%% @doc Post a message.
-spec post_message(Token :: string(), Channel :: string(), Message :: string()) -> http_response().
post_message(Token, Channel, Message) ->
    request:slack("chat.postMessage", [{"token", Token},{"channel", Channel},{"text", Message}]).
